...
Results are written to `data/output/sample/`, including `rendered.mp4`, `landmark_frames.json`, `landmark_streams.json`, `kinematic_frames.json`, keyboard calibration files, and `process.log`. `landmark_streams.json` groups observations by stable landmark ID and records an entry for every frame, including explicit missing detections. `kinematic_frames.json` contains the registered position, derivative, axis, plane, joint-angle, and keyboard-height metrics for each frame; unavailable values are explicit `null` entries. To use different paths, edit the `input_video` and `output_dir` values in `main.py`. Keyboard dimensions can be changed in `config/default.yaml`.
